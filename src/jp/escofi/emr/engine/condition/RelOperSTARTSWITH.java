 ...
 * @author EBS
 */
public final class RelOperSTARTSWITH extends AbstractSingleCondition {

	/**
	 * コンストラクタ
	 * @param items	本演算子のオペランドリスト
	 */
	public RelOperSTARTSWITH(List<AbstractOperand> items) {
		super(items);
	}

	/**
	 * 左右オペランド（String）のSTARTSWITH演算を行う。
	 * 左オペランドが右オペランドから開始する場合、TRUEを返す。
	...
	public void toDump(XMLWriter writer,
			...
			throws SAXException {
		writer.startElement(ElementType.START_SWITH.toString());
		...
		writer.endElement(ElementType.START_SWITH.toString());
	}
}
