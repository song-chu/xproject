 ...
 * @author EBS
 */
public abstract class AbstractRelOperNOTSUBSET extends AbstractSingleCondition {

	/**
	 * コンストラクタ。
	 * @param items  本演算子のオペランドリスト
	 */
	public AbstractRelOperNOTSUBSET(List<AbstractOperand> items) {
		super(items);
	}

	/**
	 * 関係演算NOTSUBSETの条件式を書き出す。
	 *
	 * @param writer ライター
	...
	public void toDump(XMLWriter writer,
			...
			throws SAXException {
		writer.startElement(ElementType.NOT_SUBSET.toString());
		...
		writer.endElement(ElementType.NOT_SUBSET.toString());
	}
