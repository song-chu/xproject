 ...
 * @author EBS
 */
public abstract class AbstractRelOperNOTEQ extends AbstractSingleCondition {

	/**
	 * 親クラスのコンストラクタを呼び出し、引数を渡す。
	 * @param items 本演算子のオペランドリスト
	 */
	public AbstractRelOperNOTEQ(List<AbstractOperand> items) {
		super(items);
	}
	...
	public void toDump(XMLWriter writer,
			...
			throws SAXException {
		writer.startElement(ElementType.NOT_EQ.toString());
		...
		writer.endElement(ElementType.NOT_EQ.toString());
	}
}
