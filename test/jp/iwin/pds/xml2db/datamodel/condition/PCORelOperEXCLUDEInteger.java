 ...
 */
public final class PCORelOperEXCLUDEInteger extends PCOARelOperEXCLUDE {

	/**
	 * コンストラクタ
	 * @param items	本演算子のオペランドリスト
	 */
	public PCORelOperEXCLUDEInteger(List<PCOAOperand> items) {
		super(items);
	}

	/**
	 * 左中右オペランド(Integer)のEXCLUDE演算を行う。Integer.compareTo()を利用する。<br>
	...
	public boolean judge(Map<String, Object> argItems) {
		...
		Integer sourceS = (Integer) assign(this.items.get(0), argItems);
		Integer target = (Integer) assign(this.items.get(1), argItems);
		Integer sourceL = (Integer) assign(this.items.get(2), argItems);
