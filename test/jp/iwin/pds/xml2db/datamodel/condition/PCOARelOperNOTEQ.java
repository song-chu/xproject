 ...
 */
public abstract class PCOARelOperNOTEQ extends PCOASingleCondition {

	/**
	 * 親クラスのコンストラクタを呼び出し、引数を渡す。
	 * @param items 本演算子のオペランドリスト
	 */
	public PCOARelOperNOTEQ(List<PCOAOperand> items) {
		super(items);
	}
	...
	public void toDump(PUTXMLWriter writer,
			...
			throws SAXException {
		writer.startElement(PCTElementType.NOTEQ.toString());
		...
		writer.endElement(PCTElementType.NOTEQ.toString());
	}
}
