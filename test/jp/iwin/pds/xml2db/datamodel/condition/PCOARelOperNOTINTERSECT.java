 ...
 */
public abstract class PCOARelOperNOTINTERSECT extends PCOASingleCondition {

	/**
	 * コンストラクタ。
	 * @param items  本演算子のオペランドリスト
	 */
	public PCOARelOperNOTINTERSECT(List<PCOAOperand> items) {
		super(items);
	}

	/**
	 * 関係演算NOTINTERSECTの条件式を書き出す。
	 *
	 * @param writer ライター
	...
	public void toDump(PUTXMLWriter writer,
			...
			throws SAXException {
		writer.startElement(PCTElementType.NOTINTERSECT.toString());
		...
		writer.endElement(PCTElementType.NOTINTERSECT.toString());
	}
}
